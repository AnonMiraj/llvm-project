...
LIBC_INLINE static constexpr bool is_odd_integer(float x) {
...
LIBC_INLINE static constexpr bool is_integer(float x) {
  using FPBits = typename fputil::FPBits<float>;
  uint32_t x_u = cpp::bit_cast<uint32_t>(x);
  int32_t x_e =
      static_cast<int32_t>((x_u & FPBits::EXP_MASK) >> FPBits::FRACTION_LEN);
...
LIBC_INLINE static constexpr bool larger_exponent(double a, double b) {
...
LIBC_INLINE static double powf_double_double(int idx_x, double dx, double y6,
                                             double lo6_hi,
